...
def run_STdGCN(paths,
    ...
    # 以下有修改
    sc_path = paths['output_path']
    output_path = paths['output_path']
    sc_adata = sc.read_h5ad(paths['sc_path'])
    ...
    #以下有修改
    Spatial = sc.read_h5ad(paths['ST_path'])
    ...
    # 有修改
    selected_genes = set(ST_adata_filter.var_names).intersection(set(selected_genes))
    print('selected_genes number:', len(selected_genes))
    ...
    if OTmapping == True:
        Xs = pseudo_adata_norm.X
        Xt = ST_adata_filter_norm.X
        ot_emd = ot.da.EMDTransport()
        ot_emd.fit(Xs=Xs, Xt=Xt)
        transp_Xs_emd = ot_emd.transform(Xs=Xs)
        pseudo_adata_norm.X = transp_Xs_emd
    else:
        pass
    ...
    '''
    A_intra_space = pd.DataFrame(np.zeros([1000, 1000]))
