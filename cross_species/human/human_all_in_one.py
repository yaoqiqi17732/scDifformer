...
input_dir = f"{home_path}/human/input"
output_dir = f"{home_path}/human/output/{current_time}"
...
# Tokenizer parm
GENE_MEDIAN_FILE = f"{home_path}/material/gene_median_dictionary.pkl"
...
def process_gene_mapping(file_path: str) -> None:
    ...
    try:
        mus_human_dict, _ = load_mapping_data()
        adata.var_names = list(adata.var["feature_name"])
        retain_gene_set = set(mus_human_dict.values()).intersection(adata.var_names)
